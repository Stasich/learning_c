...
class bMoney {
    ...
    public:
        ...
        friend bMoney round ( const bMoney& ); // округление числа.
};
...
int main()
{
    ...
    while ( ch != 'n' ) {
        ...
        cout << "Округление "; b3.show(); b3 = round( b3 ); cout << " = "; b3.show(); cout << endl << endl;
...
bMoney round ( const bMoney& bm ){
    long double* ld;
    long double temp = modfl ( bm.number, ld );
    if ( temp < 0.50 )
        return bMoney ( bm.number );
    else
        return bMoney ( bm.number + 1 );
}